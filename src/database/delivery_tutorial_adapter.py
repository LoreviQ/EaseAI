...
from src.types import DeliveryTutorial
...
class DeliveryTutorialAdapter:
    ...
    def get_delivery_tutorial(self, project_id: UUID) -> Optional[DeliveryTutorial]:
        tutorial = (
            self.session.query(DeliveryTutorialORM)
            .filter(DeliveryTutorialORM.project_id == project_id)
            .first()
        )
        return tutorial.domain if tutorial else None
